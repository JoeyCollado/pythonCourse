...
class QuizGenerator:
    def __init__(self, window):
        ...
        self.selected_subject.set("Select Subject")  # Set the default option
        self.subject_menu = tk.OptionMenu(self.start_frame, self.selected_subject, *subject_questions.keys())
        self.canvas.create_window(center_x, center_y + 150, window=self.subject_menu)  # Position the dropdown menu

        # Start button to begin the quiz
        self.start_button = tk.Button(self.start_frame, text="Start the quiz game", font=("System", 20), command=self.show_quiz)
        self.canvas.create_window(center_x, center_y + 100, window=self.start_button)  # Position the button below the dropdown
        ...
        self.remaining_questions = []  # List to hold questions for the selected subject
    ...
    def show_quiz(self):
        ...
        self.remaining_questions = subject_questions[selected_subject].copy()  # Copy the selected subject's questions
    ...
    def new_question(self):
        ...
        if not self.remaining_questions:
            self.remaining_questions = subject_questions[self.selected_subject.get()].copy()  # Reset questions if all have been used
    ...
    def check_answer(self, i):
        ...
        if selected_answer == self.correct_answer:
            ...
            self.new_question_button.config(state=tk.DISABLED)  # Disable the new question button when an incorrect answer is selected

    def restart_game(self):
        """Function to restart the game"""
        if self.score > self.high_score:
            self.high_score = self.score  # Update the highest score if the current score is higher
        ...
        self.score_label.config(text=f"Score: {self.score}")  # Update the score label
        self.remaining_questions = subject_questions[self.selected_subject.get()].copy()  # Reset the list of remaining questions
